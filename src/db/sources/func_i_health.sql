CREATE OR REPLACE FUNCTION public.i_health(h_type_id_val integer, amount_val integer, cur_val integer, oper_date date, oper_type text, src_card integer, comm_val text)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    IF h_type_id_val IS NULL THEN
        RAISE EXCEPTION 'Invalid health type ID: %', h_type_id_val;
    END IF;
